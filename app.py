...
【搭配规则】：
        1. 💰 **严控预算（最高优先级）**：推荐方案总价**必须**控制在预算的 **95%~100%** 之间（即 ¥{int(total_budget*0.95):,}~¥{total_budget:,}），**严格禁止超预算**！如果候选产品组合超出预算，必须减少品类或选择更低价的产品，直到总价不超过 ¥{total_budget:,}。
...
- 预算：¥{total_budget:,}（**总价不得超过预算¥{total_budget:,}**，严格禁止超预算）
